...
{
    public class LamaCompanyProfileSpecValidator : AbstractValidator<LamaCompanyProfileSpecDto>
    {
        public LamaCompanyProfileSpecValidator()
        {
            RuleFor(x => x.Users)
               .NotEmpty()
               .WithMessage("You should add at least one user");

            RuleFor(x => x.Users.Select(e => e.Email))
            ...
            RuleFor(x => x.Id)
              .NotNull()
              .Must(x => x != default)
              .WithMessage("Invalid lama company id");
        }
